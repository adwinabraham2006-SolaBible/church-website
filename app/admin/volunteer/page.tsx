...
export default function VolunteerAdminPage() {
  ...
  const [files, setFiles] = useState<VolunteerFile[]>([]);
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/admin/volunteer').then(r => r.json()),
      fetch('/api/admin/volunteer/files').then(r => r.json()),
  ...
  const uploadFile = async (e: React.ChangeEvent<HTMLInputElement>, type: 'photo' | 'pdf') => {
    ...
    formData.append('bucket', 'volunteer');
    ...
    const addRes = await fetch('/api/admin/volunteer/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deleteFile = async (id: string) => {
    ...
    const res = await fetch(`/api/admin/volunteer/files/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <h1 className="text-2xl font-bold text-gray-900">Volunteer Page</h1>
      ...
      <div className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <textarea
          ...
          rows={8}
          placeholder="Describe volunteer opportunities at Sola Bible Church..."
